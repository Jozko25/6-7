...
import { updateVehicleSchema, type UpdateVehicleInput } from "@/lib/validators/vehicle";
...
export function EditVehicleModal({ isOpen, onClose, onSuccess, vehicleId }: EditVehicleModalProps) {
  const [uploadedImages, setUploadedImages] = useState<string[]>([]);
  const [isUploading, setIsUploading] = useState(false);
  const { toast } = useToast();

  const { data: vehicle, isLoading: isLoadingVehicle } = trpc.vehicles.getById.useQuery(
    { id: vehicleId },
    { enabled: isOpen }
  );
  ...
  useEffect(() => {
    if (vehicle) {
      ...
      setValue("mileage", Number(vehicle.mileage));
  ...
  const updateMutation = trpc.vehicles.update.useMutation({
    ...
    onError: (error) => {
      toast({
        ...
        description: error.message || "An error occurred while updating the vehicle",
        variant: "destructive",
      });
    },
  });
    ...
    <Dialog open={isOpen} onOpenChange={(open) => (!open ? onClose() : null)}>
      <DialogContent className="sm:max-w-[760px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update the vehicle details and images.</DialogDescription>
        </DialogHeader>

        {isLoadingVehicle ? (
          <div className="py-10 text-center">
            <Loader2 className="h-6 w-6 animate-spin mx-auto text-muted-foreground" />
            <p className="mt-3 text-sm text-muted-foreground">Loading vehicle data...</p>
          </div>
        ) : (
